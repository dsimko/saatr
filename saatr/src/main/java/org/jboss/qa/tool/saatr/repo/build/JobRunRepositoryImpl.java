...
@Component
@Slf4j
class JobRunRepositoryImpl implements JobRunRepositoryCustom {

    private final MongoTemplate template;

    @Autowired
    public JobRunRepositoryImpl(MongoTemplate template) {
        this.template = template;
    }

    @Override
    public Iterator<JobRun> query(long first, long count, JobRunFilter filter) {
        final Query query = new Query();
        query.limit((int) count);
        query.skip((int) first);
        query.with(new Sort(Sort.Direction.DESC, "id"));
        query.addCriteria(createCriteria(filter, false));
        return template.find(query, JobRun.class).iterator();
    }

    @Override
    public long count(JobRunFilter filter) {
        return template.count(Query.query(createCriteria(filter, false)), JobRun.class);
    }
    ...
    private Criteria createCriteria(JobRunFilter filter, boolean convertoToBson, Criteria... additionaleCriterias) {
